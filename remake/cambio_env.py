...
from util import build_deck, CambioState, est_hand_value, known_ratio, est_state_value
...
class CambioEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = self._calculate_reward(prev_value, cur_value)
        info['valid_actions'] = self._get_valid_actions()
    ...
    def step_opponent(self):
        ...
        if self.open_action is not None:
            callback_action = agent.prompt_callback(self.known_hands[opponent], self.open_action, valid_actions=self._get_valid_actions())
    ...
    def _calculate_reward(self, prev_value, cur_value):
        """Calculate reward based on state value difference and game outcome"""
        state_reward = (cur_value - prev_value) * 2.0

        # Additional signals for good/bad situations
        player_hand_value = est_hand_value(self.known_hands[0][0])
        opponent_hand_value = est_hand_value(self.known_hands[0][1])

        value_diff_reward = 0
        if player_hand_value < opponent_hand_value:
            value_diff_reward = 0.5
        player_knowledge = known_ratio(self.known_hands[0][0])
        opponent_knowledge = known_ratio(self.known_hands[0][1])
        knowledge_reward = (player_knowledge - opponent_knowledge) * 0.5
        ...
        if self.game_over:
            ...
            if agent_score < opp_score:
                return 10.0
            elif agent_score == opp_score:  # Tie
                return 1.0
            else:
                return -5.0

        combined_reward = state_reward + value_diff_reward + knowledge_reward

        if self.game_state is CambioState.CALLED or self.game_state is CambioState.LAST_TURN:
            combined_reward *= 1.5
    ...
    def _get_observation(self):
        ...
        hand_action = np.zeros(5)
        hand_action[self.hand[0] if self.hand else -1] = 1
        discard = np.array([self.discard_pile[-1][1] / 13.0 if self.discard_pile else -1])
        ...
        if self.game_state == CambioState.NOT_CALLED:
            ...
            game_state[2] = 1.0
        open_action = np.array([self.open_action / 4.0 if self.open_action else -1], dtype=np.float32)
